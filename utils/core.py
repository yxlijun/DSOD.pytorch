...
def anchors(config):
    ...
    cfg['min_sizes'] = config.anchor_config.min_sizes
    cfg['max_sizes'] = config.anchor_config.max_sizes
...
def adjust_learning_rate(optimizer, epoch, cfg, dataset):
    lr_config = {'VOC': cfg.train_cfg.step_lr.VOC,
                 'COCO': cfg.train_cfg.step_lr.COCO}
    _lr_config = lr_config[dataset]
    global lr
    if epoch in _lr_config:
        i = _lr_config.index(epoch) + 1
        lr = pow(cfg.train_cfg.lr, i + 1) if pow(cfg.train_cfg.lr,
                                                 i + 1) > cfg.train_cfg.end_lr else cfg.train_cfg.end_lr
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO', epoch=10):
    ...
    if final:
        torch.save(net.state_dict(), cfg.model.weights_save +
                   'Final_STDN_{}_size{}.pth'.format(datasetname, cfg.model.input_size))
    else:
        torch.save(net.state_dict(), cfg.model.weights_save +
                   'STDN_{}_size{}_epoch{}.pth'.format(datasetname, cfg.model.input_size, epoch))
